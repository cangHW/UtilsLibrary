...
import com.perfect.library.adapter.baseAdapter.BaseListViewAdapter;
 ...
 * Time : 2016/12/9 15:10.
 * Function :对BaseListViewAdapter的功能方面扩充
 */

public abstract class FunctionListViewAdapter<T extends Object, K extends BaseListViewAdapter.BaseViewHolder> extends BaseListViewAdapter<T, K> {

    public FunctionListViewAdapter(Context context) {
        super(context);
    }

    public FunctionListViewAdapter(Context context, List<T> data) {
        super(context, data);
    }
    ...
    private void addDatas(int location, List<T> datas) {
        this.getDatas().addAll(location, datas);
        this.setNotify();
    }
     ...
     * 通过位置删除item
     *
     * @param location 需要删除的数据位置
     * @throws IndexOutOfBoundsException location不能超出数据集的范围
     */
    public void deleteItemsFromLocation(int location){
        ExceptionUtils.checkLocationIsLegal(location,getCount());
        this.getDatas().remove(location);
        this.setNotify();
    }
    ...
    public T findDataByLocation(int location) {
        ExceptionUtils.checkLocationIsLegal(location,getCount());
        return this.getDatas().get(location);
    }
    ...
    public List<T> findDatasByLocations(int... ints) {
        ...
        for (int location : ints) {
            ExceptionUtils.checkLocationIsLegal(location,getCount());
            list.add(this.getDatas().get(location));
        }
        return list;
    }
     ...
     * 数据集变化，刷新整个控件
     */
    public void setNotifyInvalidated() {
